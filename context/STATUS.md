...
**Frontier key:** `ANTHROPIC_API_KEY=Bearer <kie.ai key>` set in both `~/.hermes/.env` (laptop) and `/root/.hermes/.env` (VPS). Routes Claude via `https://api.kie.ai/claude` (model `Claude-Opus-4-8`).
